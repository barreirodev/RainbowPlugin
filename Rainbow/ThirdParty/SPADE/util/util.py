...
class Colorize(object):
    ...
    def __call__(self, gray_image):
        ...
        #save_cmap_to_txt(self.cmap, 'srgbmap.txt')
...
#Added to kAIros
def save_cmap_to_txt(cmap, filename):
    with open(filename, 'w') as file:
            for i in range(cmap.shape[0]):
                r, g, b = cmap[i]
                r = int(np.round(pow((r/255),(1/2.2))*255))
                g = int(np.round(pow((g/255),(1/2.2))*255))
                b = int(np.round(pow((b/255),(1/2.2))*255))
